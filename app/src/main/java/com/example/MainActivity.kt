...
class MainActivity : AppCompatActivity() {
    ...
    companion object {
        private const val PREFS_NAME = "ptp_playback_prefs"
    ...
    private fun setupAudioControls() {
        ...
        btnAudioShuffle.setOnClickListener {
            ...
            updateAudioShuffleButtonUi()
            saveAudioShufflePref()
        }
    ...
    private fun updateAudioShuffleButtonUi() {
        if (::btnAudioShuffle.isInitialized) {
            btnAudioShuffle.text = if (isAudioShuffleEnabled) "🔀 SHUF: ON" else "🔀 SHUF"
        }
    }
    ...
    private fun handleRewind10Action() {
        if (currentScreen == Screen.VIDEO_PLAYER) {
            ...
            val pos = (cur - 10000).coerceAtLeast(0)
            videoView.seekTo(pos)
            tvVideoTimeCurrent.text = formatTime(pos)
            val dur = videoView.duration
            if (dur > 0) {
                sbVideoSeek.progress = (pos.toLong() * 1000L / dur).toInt()
            }
            showVideoHud()
        } else if (currentScreen == Screen.AUDIO_PLAYER) {
            audioPlayer?.let { mp ->
                try {
                    ...
                    val pos = (cur - 10000).coerceAtLeast(0)
                    mp.seekTo(pos)
                    tvAudioTimeCurrent.text = formatTime(pos)
                    val dur = mp.duration
                    if (dur > 0) {
                        progressAudioSeek.progress = (pos.toLong() * 1000L / dur).toInt()
                    }
                    showAudioHud()
                } catch (_: Exception) {}
            }
    ...
    private fun handleForward10Action() {
        if (currentScreen == Screen.VIDEO_PLAYER) {
            ...
            val pos = if (dur > 0) target.coerceAtMost(dur) else target
            videoView.seekTo(pos)
            tvVideoTimeCurrent.text = formatTime(pos)
            if (dur > 0) {
                sbVideoSeek.progress = (pos.toLong() * 1000L / dur).toInt()
    ...
    private fun handlePreviousAction() {
        if (currentScreen == Screen.VIDEO_PLAYER) {
            ...
            if (displayedAudioList.isEmpty()) return
            if (isAudioShuffleEnabled && displayedAudioList.size > 1) {
                var prevIdx = Random.nextInt(displayedAudioList.size)
                if (prevIdx == currentAudioIndex) {
                    prevIdx = if (currentAudioIndex > 0) currentAudioIndex - 1 else displayedAudioList.size - 1
                }
                playAudioAtIndex(prevIdx)
    ...
    private fun handleNextAction() {
        if (currentScreen == Screen.VIDEO_PLAYER) {
            ...
            if (displayedAudioList.isEmpty()) return
            if (isAudioShuffleEnabled && displayedAudioList.size > 1) {
                var nextIdx = Random.nextInt(displayedAudioList.size)
                if (nextIdx == currentAudioIndex) {
                    nextIdx = (currentAudioIndex + 1) % displayedAudioList.size
                }
                ...
                initJob = lifecycleScope.launch {
                    try {
                        ...
                        if (success) {
                            ...
                            updateMediaCounts()
                            val resumed = checkAndResumeLastPlayed()
                            if (!resumed) {
    ...
    fun playVideoAtIndex(index: Int, initialPositionMs: Long = 0L) {
        if (index < 0 || index >= displayedVideoList.size) return
        currentVideoIndex = index
        val item = displayedVideoList[index]
        saveLastPlayedState(MEDIA_TYPE_VIDEO, item.filename, initialPositionMs)
        startVideoPlayback(item, initialPositionMs)
    }

    private fun startVideoPlayback(item: PtpMediaItem, initialPositionMs: Long = 0L) {
        val client = repository.client ?: return
        val sessionId = ++currentVideoSessionId

        // Cancel previous session, jobs, and player immediately
        ...
        tvVideoTimeCurrent.text = formatTime(initialPositionMs.toInt())
        ...
        lifecycleScope.launch {
            ...
            if (sessionId == currentVideoSessionId) {
                videoView.setPlaybackSession(session = session, initialPositionMs = initialPositionMs)
            }
        }
    }
                ...
                try {
                    if (videoView.isPlaying && !isVideoTracking) {
                        ...
                        if (total > 0) {
                            ...
                            if (now - lastSavedTimestamp > 2500) {
                                lastSavedTimestamp = now
                                saveLastPlayedPosition(current.toLong())
                            }
                        }
                    }
                } catch (_: Exception) {}
    ...
    fun playAudioAtIndex(index: Int, initialPositionMs: Long = 0L) {
        if (index < 0 || index >= displayedAudioList.size) return
        currentAudioIndex = index
        val item = displayedAudioList[index]
        saveLastPlayedState(MEDIA_TYPE_AUDIO, item.filename, initialPositionMs)
        startAudioPlayback(item, initialPositionMs)
    }

    private fun startAudioPlayback(item: PtpMediaItem, initialPositionMs: Long = 0L) {
        val client = repository.client ?: return
        ...
        tvAudioTimeCurrent.text = formatTime(initialPositionMs.toInt())
            ...
            if (file != null && file.exists() && file.length() > 0) {
                try {
                    ...
                    mp.setOnPreparedListener { player ->
                        ...
                        if (initialPositionMs > 0 && initialPositionMs < player.duration) {
                        ...
                        updateMediaSessionState(PlaybackStateCompat.STATE_PLAYING, player.currentPosition.toLong())
    ...
    private fun saveLastPlayedState(type: String, filename: String, positionMs: Long) {
        try {
            getSharedPreferences(PREFS_NAME, MODE_PRIVATE).edit()
                .putString(KEY_LAST_MEDIA_TYPE, type)
    ...
    private fun saveLastPlayedPosition(positionMs: Long) {
        try {
            getSharedPreferences(PREFS_NAME, MODE_PRIVATE).edit()
                .putLong(KEY_LAST_POSITION_MS, positionMs)
                .apply()
    ...
    private fun flushCurrentPosition() {
        try {
            if (currentScreen == Screen.VIDEO_PLAYER) {
                val cur = videoView.currentPosition.toLong()
                if (cur > 0) {
                    saveLastPlayedPosition(cur)
                }
            } else if (currentScreen == Screen.AUDIO_PLAYER) {
                audioPlayer?.let { mp ->
    ...
    private fun checkAndResumeLastPlayed(): Boolean {
        try {
            val prefs = getSharedPreferences(PREFS_NAME, MODE_PRIVATE)
            val mediaType = prefs.getString(KEY_LAST_MEDIA_TYPE, null) ?: return false
            val filename = prefs.getString(KEY_LAST_FILENAME, null) ?: return false
            val positionMs = prefs.getLong(KEY_LAST_POSITION_MS, 0L)

            if (mediaType == MEDIA_TYPE_VIDEO) {
                val idx = displayedVideoList.indexOfFirst { it.filename == filename }
                if (idx != -1) {
                    playVideoAtIndex(idx, positionMs)
                    return true
                }
            } else if (mediaType == MEDIA_TYPE_AUDIO) {
                val idx = displayedAudioList.indexOfFirst { it.filename == filename }
                if (idx != -1) {
                    playAudioAtIndex(idx, positionMs)
                    return true
                }
            }
        } catch (e: Exception) {
            Log.w(PtpConstants.TAG, "Error checking/resuming last played item", e)
        }
        return false
    }

    private fun saveAudioShufflePref() {
        try {
            getSharedPreferences(PREFS_NAME, MODE_PRIVATE).edit()
                .putBoolean(KEY_AUDIO_SHUFFLE, isAudioShuffleEnabled)
                .apply()
        } catch (_: Exception) {}
    }

    private fun loadAudioShufflePref() {
        try {
            isAudioShuffleEnabled = getSharedPreferences(PREFS_NAME, MODE_PRIVATE)
                .getBoolean(KEY_AUDIO_SHUFFLE, false)
            updateAudioShuffleButtonUi()
        } catch (_: Exception) {}
    }
}
